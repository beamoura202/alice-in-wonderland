/**
 * Fisheye effect implementation based on Eric Leong's fisheye.js library.
 * Modified for use with Alice in Wonderland project.
 * 
 * The MIT License (MIT)
...
\
                gl_FragColor = vec4(0.208, 0.169, 0.161, 1.0); /* Start with #352b29 background */\
\
...
\
                if (redCoord.x >= 0.0 && redCoord.x <= 1.0 && redCoord.y >= 0.0 && redCoord.y <= 1.0) {\
                    vec4 redSample = texture2D(uImage, redCoord);\
                    gl_FragColor.r = redSample.r;\
                    if (uDistortion.r != 0.0) { /* Only modify alpha if distortion is active */\
                        gl_FragColor.a = mix(gl_FragColor.a, redSample.a, 0.33);\
                    }\
                }\
\
...
\
                if (greenCoord.x >= 0.0 && greenCoord.x <= 1.0 && greenCoord.y >= 0.0 && greenCoord.y <= 1.0) {\
                    vec4 greenSample = texture2D(uImage, greenCoord);\
                    gl_FragColor.g = greenSample.g;\
                    if (uDistortion.g != 0.0) { /* Only modify alpha if distortion is active */\
                        gl_FragColor.a = mix(gl_FragColor.a, greenSample.a, 0.33);\
                    }\
                }\
\
...
\
                if (blueCoord.x >= 0.0 && blueCoord.x <= 1.0 && blueCoord.y >= 0.0 && blueCoord.y <= 1.0) {\
                    vec4 blueSample = texture2D(uImage, blueCoord);\
                    gl_FragColor.b = blueSample.b;\
                    if (uDistortion.b != 0.0) { /* Only modify alpha if distortion is active */\
                        gl_FragColor.a = mix(gl_FragColor.a, blueSample.a, 0.33);\
                    }\
                }\
\
                /* If no distortion applied, just use original image */\
                if (uDistortion.r == 0.0 && uDistortion.g == 0.0 && uDistortion.b == 0.0) {\
...
Fisheye.prototype.clear = function() {
    this.gl.clearColor(0.208, 0.169, 0.161, 1.0); // #352b29
...
Fisheye.prototype.draw = function(image) {
    this.clear(); // This will now use the correct background color
